...
class CompressionModel(nn.Module):
    def __init__(self, z_channel, extra_qp=0, y_ch=None, scale_floor=0.11):
        super().__init__()
    ...
    #视频uf的四叉树模型
    @staticmethod
    def get_one_mask(pattern, height, width, dtype, device):
        mask = torch.tensor(pattern, dtype=dtype, device=device)
        mask = mask.repeat((height + 1) // 2, (width + 1) // 2)
        mask = mask[:height, :width]
        return mask[None, None, :, :]

    def get_quadtree_masks(self, batch, channel, height, width, dtype, device):
    ...
    #四叉树压缩
    def compress_prior_uf_quadtree_sem(self, y, common_params):
        y, q_dec, _, _ = self.separate_prior_for_encoding(common_params, y)
        batch, channel, height, width = y.shape
        masks = self.get_quadtree_masks(batch, channel, height, width, y.dtype, y.device)

        y_q, scales, y_hat, _ = self.streamlinedentropy.quantize(
            y, common_params, masks,
            force_zero_thres=self.gaussian_encoder.force_zero_thres,
            scale_min=self.gaussian_encoder.scale_min,
            scale_max=self.gaussian_encoder.scale_max,
        )

        y_hat = y_hat * q_dec
        return y_q, scales, y_hat

    def decompress_prior_uf_quadtree_sem(self, common_params):
        infos = self.decompress_prior_uf_quadtree_sem_part1(common_params)
        return self.decompress_prior_uf_quadtree_sem_part2(common_params, infos)

    def decompress_prior_uf_quadtree_sem_part1(self, common_params):
        q_dec, _, _ = self.separate_prior_for_decoding(common_params)

        scales = self.streamlinedentropy.estimate_scales(common_params)
        scales_for_indexes = scales.clone()
        indexes, skip_cond = self.gaussian_encoder.build_indexes_decoder(scales_for_indexes)
    ...
    def decompress_prior_uf_quadtree_sem_part2(self, common_params, infos):
        scales = infos["scales"]
        dtype = scales.dtype
        device = scales.device

        y_q = self.gaussian_encoder.get_y(
            scales.shape,
            scales.numel(),
            dtype,
            device,
        ...
        batch, channel, height, width = y_q.shape
        masks = self.get_quadtree_masks(batch, channel, height, width, dtype, device)

        y_hat, _, _ = self.streamlinedentropy.restore(
            y_q, common_params, masks
        )

        return y_hat * infos["q_dec"]


    #将参数params拆分成几个部分（i帧图像使用）
