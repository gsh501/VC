...
def get_expected_frame_names(expected_frames):
    return {f"im{idx:03d}" for idx in range(1, expected_frames + 1)}


def get_frame_stems(frames):
    stems = set()
    for frame in frames:
        match = FRAME_RE.match(frame.name)
        if match:
            stems.add(f"im{int(match.group(1)):03d}")
    return stems
...
def split_counts(total, ratios):
    ...
    if total >= len(positive_indices):
        for idx in positive_indices:
            if counts[idx] == 0:
                donor = max(positive_indices, key=lambda donor_idx: counts[donor_idx])
                if counts[donor] <= 1:
                    break
                counts[donor] -= 1
                counts[idx] = 1
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Directory for the generated filelists.",
    )
    parser.add_argument("--train-output", type=str, default="train_filelist_phase2.txt")
    parser.add_argument("--val-output", type=str, default="val_filelist_phase2.txt")
    parser.add_argument("--test-output", type=str, default="test_filelist_phase2.txt")
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Expected frame count in every leaf GOP folder.",
    )
    parser.add_argument(
        "--drop-incomplete",
        action="store_true",
...
def main():
    ...
    expected_names = get_expected_frame_names(args.expected_frames)
    incomplete = []
    complete_sequence_frames = {}
    for folder, frames in sequence_frames.items():
        frame_names = get_frame_stems(frames)
        missing = sorted(expected_names - frame_names)
        extra = sorted(frame_names - expected_names)
        wrong_count = len(frames) != args.expected_frames
        if missing or extra or wrong_count:
            incomplete.append((folder, missing, extra, len(frames)))
            continue
        complete_sequence_frames[folder] = frames

    if incomplete and not args.drop_incomplete:
        for folder, missing, extra, frame_count in incomplete[:20]:
            print(f"incomplete GOP: {folder}")
    ...
    if not sequence_frames:
        raise RuntimeError("No complete 32-frame GOP folders were found.")

    splits = split_sequences(
        list(sequence_frames.keys()),
        args.train_ratio,
        args.val_ratio,
        args.test_ratio,
        args.seed,
    ...
    print(f"complete_gops: {len(sequence_frames)}")
    print(f"frames: {total_frames}")
    print(f"incomplete_gops: {len(incomplete)}")
    if args.drop_incomplete and incomplete:
        print("drop_incomplete: yes")

    for split_name in ("train", "val", "test"):
        print(
            f"{split_name}: "
            f"gops={len(splits[split_name])}, "
