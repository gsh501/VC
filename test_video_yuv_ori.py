...
def psnr(x_hat: torch.Tensor, y: np.ndarray, u: np.ndarray, v: np.ndarray) -> float:
    ...
    # Convert reconstruction to YUV420 planes (torch -> numpy)
    y_rec, uv_rec = yuv_444_to_420(x_hat)
    y_rec = torch.clamp(y_rec * 255, 0, 255).squeeze(0).cpu().numpy()
    # y_rec shape -> (1, H, W) -> take channel 0
    y_rec = y_rec[0]
    uv_rec = torch.clamp(uv_rec * 255, 0, 255).squeeze(0).cpu().numpy()
    u_rec, v_rec = uv_rec[0], uv_rec[1]

    # Ground-truth may be numpy arrays (from reader) or torch tensors.
    def to_np_plane(x):
        if isinstance(x, torch.Tensor):
            arr = x.squeeze(0).cpu().numpy()
            # if channel-first (1,H,W) -> take channel 0
            if arr.ndim == 3:
                return arr[0]
            return arr
        else:
            # numpy
            arr = np.squeeze(x, axis=0) if getattr(x, 'ndim', 0) >= 3 else np.squeeze(x)
            return arr
    ...
    # Channel PSNRs
    psnr_y = calc_psnr(y_gt, y_rec)
    psnr_u = calc_psnr(u_gt, u_rec)
    psnr_v = calc_psnr(v_gt, v_rec)
...
def Var(x):
    """Return a Variable wrapping `x`, moved to CUDA if available."""
...
class SingleYuvDataset(Dataset):
    ...
    def __getitem__(self, _):
        ...
        return gop, self.y_list, self.uv_list, self.name
...
def Test(epoch, model_i, test_dataloader, model, criterion, test_num,QP):
    ...
    with torch.no_grad():
        for gop, y_list, uv_list, name in test_dataloader:
            gop = gop.squeeze(1)
            gop = Var(gop)
            ...
            for j in range(0, test_num ):
                ...
                if j==0:
                    if qs_global > 63:
                        out = model_i.compress_(cur, 63)
                    else:
                        out = model_i.compress_(cur, qs_global)

                    # P-frames
                    model.clear_dpb()
                    model.add_ref_frame(None, out["x_hat"])
                else:
                    if j%32== 1:
...
def main():
    ...
    parser.add_argument("--yuv",     default="/home/admin1/Data/water_date_test/test_data_yuv/1280_720/009.yuv", help=".yuv file path")
    ...
    parser.add_argument("--ckpt_p",  default="checkpoints/cvpr2025_video.pth.tar", help="DMC  checkpoint")
