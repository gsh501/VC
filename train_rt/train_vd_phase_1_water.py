...
def qp_to_lambda(qp, q_num=72, lam_min=1, lam_max=384):
    """
...
#       
        ...
        if qs_global>63:
            ref_result = i_frame_net.compress_(ref, 63)
        else:
            ref_result = i_frame_net.compress_(ref, qs_global)
   

        model.module.clear_dpb()
        model.module.add_ref_frame(None, ref_result["x_hat"])
...
def test_epoch(epoch,i_frame_net, test_dataloader, model, criterion, test_num):
    ...
    with torch.no_grad():
        for i, d in enumerate(test_dataloader):
            ...
            total_psnr = 0.0
            if qs_global>63:
                out = i_frame_net.compress_(ref, 63)
            else:
                out = i_frame_net.compress_(ref, qs_global)
            model.module.clear_dpb()
            model.module.add_ref_frame(None, out["x_hat"])
...
def parse_args(argv):
    ...
    parser.add_argument("-m", "--model", default="DMC_slf_yuv420_water", choices=["DMC"], help="Model architecture")
    ...
    parser.add_argument("-q", "--quality-level", type=int, default=11, help="Quality level")
    ...
    parser.add_argument("--checkpoint", type=str, default="pretrained/DMC_slf/1/checkpoint_vd.pth.tar", help="Path to checkpoint")
    parser.add_argument("--manyframe_epoch", type=int, default=240, help="Epoch threshold to switch to 7-frame training")
...
def main(argv):
    ...
    if args.checkpoint:
        ...
        last_epoch = max(0, checkpoint["epoch"] + 1)
