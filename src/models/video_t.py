...
class DMC(CompressionModel):
    ...
    def freeze_backbone_for_recon(self):
        """
        冻结模型中除 `recon_generation_net` 和 `q_recon` 外的所有参数。
        这样训练时只有 `self.recon_generation_net(feature, q_recon)` 和 `q_recon` 可学习，
        便于对重建网络进行水下图像增强微调。
        """
        # 先全部冻结
        ...
        # 仅解冻 recon_generation_net 的参数
        for name, param in self.recon_generation_net.named_parameters():
            param.requires_grad = True

        # 确保 q_recon 可学习（它是一个 nn.Parameter）
        try:
            self.q_recon.requires_grad = True
        except Exception:
            pass
        ...
        print(f"Freeze backbone: trainable params {trainable}/{total}")
...
# 在你的 `DMC` 类中调用这个方
    ...
    def forward(self, x, qp):
        ...
        # MS-SSIM 要求短边 > 160，小分辨率时跳过避免 AssertionError
        if min(H, W) > 160:
            ssim = self.ssim(x, x_hat)
        else:
    ...
    def forward_water(self, x, qp, gt=None):
        """
        用于水下增强的前向函数：不计算比特率损失，只返回重建图像、MSE 和 SSIM。
        与 `forward` 相同的推理流程，但省略所有 bpp/entropy 相关计算。
        如果提供了 gt，则使用 x_hat 和 gt 计算 MSE 和 SSIM；否则使用 x 和 x_hat。
        """
        device = x.device
        q_encoder = self.q_encoder[qp:qp+1, :, :, :]
        q_decoder = self.q_decoder[qp:qp+1, :, :, :]
        ...
        # 如果提供了 gt，使用 x_hat 和 gt 计算 MSE 和 SSIM；否则使用 x 和 x_hat
        if gt is not None:
            ref = gt.unsqueeze(0) if gt.dim() == 3 else gt
        else:
            ref = x
        mse = self.mse(ref, x_hat)
        mse = torch.sum(mse, dim=(1, 2, 3)) / pixel_num
        if min(H, W) > 160:
            ssim = self.ssim(ref, x_hat)
        else:
            ssim = torch.zeros(B, device=x.device, dtype=x.dtype)
